...
class Luban {
  ...
  static Future<List<int>> compressRowImage(CompressObject object) async {
    return compute(_lubanCompressRaw, object);
  }
  ...
  static List<int> _lubanCompressRaw(CompressObject object) {
    Image image = decodeImage(object.imageBytes!)!;
    final length = object.imageBytes!.length;
    print(object.imageExt!);
    bool isLandscape = false;
    const List<String> jpgSuffix = ["jpg", "jpeg", "JPG", "JPEG"];
    const List<String> pngSuffix = ["png", "PNG"];
    bool isJpg = _parseType(object.imageExt!, jpgSuffix);
    bool isPng = false;

    if (!isJpg) isPng = _parseType(object.imageExt!, pngSuffix);
    ...
    List<int>? decodedImageFile;
    if (isJpg)
      print('Decoding JPG image');
    else if (isPng)
      print('Decoding PNG image');
    else
      throw Exception("flutter_luban don't support this image type");
    ...
    if (scale <= 1 && scale > 0.5625) {
      if (fixelH < 1664) {
        if (imageSize < 150) {
          return encodeJpg(image, quality: object.quality);
        }
        size = (fixelW * fixelH) / pow(1664, 2) * 150;
        size = size < 60 ? 60 : size;
      } else if (fixelH >= 1664 && fixelH < 4990) {
    ...
    if (object.mode == CompressMode.LARGE2SMALL) {
      return _large2SmallCompressRawImage(
        image: smallerImage,
        imageBytes: decodedImageFile,
        quality: object.quality,
        targetSize: size,
        step: object.step,
        isJpg: isJpg,
      ...
      return _small2LargeCompressRawImage(
        image: smallerImage,
        imageBytes: decodedImageFile,
        quality: object.step,
        targetSize: size,
  ...
  static List<int> _large2SmallCompressRawImage({
    Image? image,
    List<int>? imageBytes,
    quality,
    targetSize,
  ...
  static List<int> _small2LargeCompressRawImage({
    Image? image,
    List<int>? imageBytes,
    quality,
    targetSize,
  ...
  static List<int> _compressRawPng({
    required Image image,
    List<int>? imageBytes,
    level,
    targetSize,
